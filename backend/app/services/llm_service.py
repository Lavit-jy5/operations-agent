...
{{导入段}}
另起 1 段写完整导入，通常 3-5 句，交代事件背景、市场反应、关键数据和文章要回答的问题；可以使用材料中的核心数据，但表达角度要和 summary 不同，不要机械复写 summary。随后进入【热点解读】。
数据自然写入句子，不使用括号式数据来源。
    ...
    return [
        ...
        },
        {
            ...
            "content": (
                ...
                "summary 只作为后台摘要，body 开头不得原样复写 summary；问候后必须另起 1 段完整导入，通常 3-5 句，交代事件背景、市场反应、关键数据和文章要回答的问题，再进入【热点解读】。导入段可以使用材料中的核心数据，但表达角度要和 summary 不同，避免机械重复。"

...
所有涉及数据的句子必须能追溯到输入数据。
数据规则：正文中的涨跌幅、规模、金额、占比、费率、权重、时间、排名等具体数字，只能来自用户输入的文字材料、图片解析材料、文档解析材料或结构化 Wind 数据；不得根据常识、记忆或推测补写。保留【数据锚点】板块用于复核，板块标题必须单独成行，下面列 2-4 条关键数据。
...
{{轻引入，可省略}}
最多 1 句轻引入，不要复写 summary 中已经出现的完整事件、行情和数据；随后直接进入【数据锚点】。
数据自然写入句子，不使用括号式数据来源。

【数据锚点】
{{小标题必须单独占一行。下方列 2-4 条最关键数据，只写材料中明确提供的数字、含义、日期/口径；来源用自然短语，例如“东吴证券统计显示”，不要写“来源：xxx”或括号来源。}}
- {{数字 + 含义 + 日期/口径}}
- {{数字 + 含义 + 日期/口径}}
- {{数字 + 含义 + 日期/口径}}
    ...
    return [
        ...
        },
        {
            ...
            "content": (
                ...
                "必须严格遵循热点文章技能包，正文包含开场、数据锚点、热点解读、后市展望、相关产品占位、互动收尾和风险提示占位。"
                ...
                "summary 只作为后台摘要，body 开头不得复写 summary；问候后最多写 1 句轻引入，随后直接进入【数据锚点】和【热点解读】，避免同一组事件、行情和数据连续讲两遍。"
                "热点文章保留【数据锚点】板块；【数据锚点】【热点解读】【后市展望】【相关产品】【风险提示】必须单独成行，下一段再写内容。"
...
def _normalize_generated_body(text: str, title: str, request: BriefGenerateRequest) -> str:
    ...
    for heading in ["【数据锚点】", "【热点解读】", "【后市展望】", "【相关产品】", "【风险提示】", "【质检备注】"]:

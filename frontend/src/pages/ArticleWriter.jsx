...
const ARTICLE_PROMPT_TEMPLATE = `你要生成一篇基金运营热点文章，参考既有样稿风格：先问候并说明作者，再进入核心内容，正文包含【数据锚点】、【热点解读】、【后市展望】、【相关产品】和【风险提示】。

写作规则：
- 文章主题可以很短，例如“黄金”“半导体”“创新药”，你需要根据材料展开成完整标题和正文。
- 作者必须带“国泰基金”前缀，例如“国泰基金武家缘”；如果用户只填姓名，请自动补成“国泰基金+姓名”。
- summary 只作为后台摘要，不要在 body 开头复写 summary；body 开场只保留“大家好，我是国泰基金XX。”和最多 1 句轻引入，随后直接进入【数据锚点】和【热点解读】，避免把同一组事件、行情和数据连续讲两遍。
- 正文要增加数据体现，优先从用户输入文字、图片识别内容、参考文档中提取可引用数据；保留【数据锚点】板块用于复核。
- 涉及行情、指数、资金、规模、费率、权重、时间、排名等具体数字时，可以把官方/机构来源自然写进句子里，不要使用括号式数据来源。
- 任何具体数字都必须来自用户上传图片或输入文字/文档，不得凭空补写、按经验补写或联网猜测。
- 【数据锚点】必须单独占一行；下方列 2-4 条最关键数据，只写材料中明确提供的数字、含义、日期/口径；不要写“来源：xxx”或括号来源。
...
function buildArticleOutputTemplate(template = "") {
  return [
    ...
    "- summary 只作为后台摘要，不要在 body 开头复写；body 问候后最多 1 句轻引入，随后直接进入【数据锚点】和【热点解读】。",

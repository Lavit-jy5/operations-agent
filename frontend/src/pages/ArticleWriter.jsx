...
- summary 只作为后台摘要，不要在 body 开头原样复写 summary；body 开场先写“大家好，我是国泰基金XX。”，下一段写一个完整导入段，通常 3-5 句，交代事件背景、市场反应、关键数据和文章要回答的问题，再进入【热点解读】。导入段可以使用材料中的核心数据，但表达角度要和 summary 不同，避免机械重复。
...
function buildArticleOutputTemplate(template = "") {
  return [
    ...
    "- summary 只作为后台摘要，不要在 body 开头原样复写；body 问候后需要写一个完整导入段，通常 3-5 句，交代事件背景、市场反应、关键数据和文章要回答的问题，再进入【热点解读】。",
